...
for i in range(len(alpha_vals)):
    for j in range(len(R0_vals)):
        ...
        condition_nums[i, j] = l_x_algo_multiplicative(
            my_pdf, K=1000, conditions=[is_real, in_bounds], is_pgf=True
        )
        variances[i, j] = variance_sim_multiplicative(
            my_pdf, trials=1000, conditions=[is_real, in_bounds]
        )
...
df_cond_nums.rename(
    columns=lambda s: R0_vals[s], index=lambda s: alpha_vals[s], inplace=True
)
