...
def solve_for_root(r, p):
    ...
    while not np.allclose(u_old, u_new, rtol=1e-07, atol=1e-12):
        u_old = u_new
        u_new = G(u_old, r, p)
...
# for i in range(len(root_lines)):
#     for j in range(len(root_lines[i])):
#         if root_lines[i][j] == (1-np.finfo(float).eps):
#             root_lines[i][j] = 1.0
...
plt.show()
# plt.savefig("/Users/mcboudre/Documents/LSD_Lab/stochastic-polys/figures/outbreak_contours_correctdist_r0_08-4_alpha_001-1_copper_"+date+".pdf", format = "pdf")
