...
ax[0].set(xlabel='R0 Values', ylabel='Dispersion Values', title='Additive Condition Numbers')

ax[1].imshow(condition_nums_multiplicative,
            extent = (min(R0_vals),max(R0_vals),min(alpha_vals),max(alpha_vals)),
             aspect = 'auto', cmap = 'Greens'
            )
ax[1].set(xlabel='R0 Values', ylabel='Dispersion Values', title='Multiplicative Condition Numbers')
plt.tight_layout()
plt.show()
