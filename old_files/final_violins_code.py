...
def violins(roots):
    ...
    #plt.savefig("box_plots_hetero_3point9_0point13.pdf", format = 'pdf')
    plt.show(ax)
...
def solvingForExpectation(params, sigma):
    ...
    expectUTrue = expectationOfU(g1True, true_root, sigma)
    expectUError = expectationOfU(g1, true_root, sigma)
    
    return expectUTrue, expectUError, true_root #, root#same thing as delta root


def solvingErroredRoot(params, sigma):
    maxk = 20
...
# expectedU, expectedUError, truth, errored_root = solvingForExpectation([r0,k], sigma = 0.1)
...
# plt.savefig("contours.pdf", format = 'pdf')
...
####### R_0 = 4 with varying k of simulated error ########
...
k_vec = np.arange(0.01, 1, 0.001)

r0_vals = np.empty((len(k_vec)))
r0_vals = 4
k_vals = np.empty((len(k_vec)))
expectedU_vals = np.empty((len(k_vec)))
exprectedUError_vals = np.empty((len(k_vec)))
root_vals = np.empty((len(k_vec)))
errored_root_vals = np.empty((1000, len(k_vec)))
...
for k in k_vec:
    ...
    for s in range(0,1000):
        errored_root_vals[s,count] = solvingErroredRoot([r0,k], sigma = 0.1)
    expectedU_vals[count], exprectedUError_vals[count], root_vals[count] = solvingForExpectation([r0, k], sigma = 0.1)
    count += 1



df_roots_errors_r0_4 = pd.DataFrame({'R0': r0_vals, 'k':k_vals, 'Roots':root_vals, 'Errored Roots': np.mean(errored_root_vals, axis = 0)}) 
df_roots_errors_r0_4["Diff"] = df_roots_errors_r0_4['Roots'] - df_roots_errors_r0_4['Errored Roots']
df_roots_errors_r0_4["Percentage Variability"] = df_roots_errors_r0_4["Diff"]/df_roots_errors_r0_4["Roots"]

plt.plot(df_roots_errors_r0_4["k"], df_roots_errors_r0_4['Percentage Variability'])
plt.xlabel("Percent Variability")
plt.title(r'Percent Variability for $R_{0} = 4$')
plt.ylabel("k, dispersion parameter")
plt.show()
plt.close()

plt.plot(df_roots_errors_r0_4["k"], df_roots_errors_r0_4['Diff'])
plt.xlabel("Difference")
plt.title(r'Difference between the True Root and the Errored Root for $R_{0} = 4$')
plt.ylabel("k, dispersion parameter")
plt.show()
plt.close()
